...
import { ShieldAlert, Filter, CheckCircle2, LucideHistory } from 'lucide-react';
            ...
            <div className="flex-grow space-y-6">
                    ...
                    <div className="flex gap-2">
                        {['Today', 'This Week', 'All Time'].map(f => (
                            <button
                                ...
                                className={`px-4 py-2 rounded-xl text-[10px] font-black uppercase tracking-widest transition-all border ${timeFilter === f ? 'bg-[var(--accent-primary)] border-[var(--accent-primary)] text-white shadow-lg' : 'bg-[var(--bg-card)] border-[var(--border-color)] text-[var(--text-muted)] hover:text-white'}`}
                            >
                                {f}
                            </button>
                        ))}
                ...
                <div className="grid grid-cols-1 gap-4">
                    {filteredLogs.length > 0 ? filteredLogs.map((log) => {
                                ...
                                <div className="flex items-center justify-between">
                                    <div className="flex items-center gap-4">
                                        ...
                                        <div>
                                            ...
                                            <p className="text-sm font-medium text-[var(--text-secondary)]">{log.action}</p>
                                        </div>
                                    </div>
                                    <div className="flex items-center gap-4">
                        ...
                        <Card className="p-12 text-center border-[var(--border-color)] bg-[var(--bg-card)]/20">
